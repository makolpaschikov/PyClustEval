...
def _registry_names(registry: Any) -> list[str]:
    """Extract names from public or private registry storage.

    Some project registries intentionally expose only get()/load().  In that
    case we inspect their dictionary fields and finally probe the known default
    entries shipped with PyClustEval.
    """
    ...
    # Named private fields used by common registry implementations.
    for attr in (
    ...
    # Generic fallback: inspect every instance attribute and use the first
    # non-empty string-keyed mapping. This covers DatasetRegistry._factories
    # and similar implementation details without requiring a project change.
    ...
    if "dataset" in registry_type:
        candidates = ("iris", "wine", "breast_cancer", "digits", "blobs", "moons")
        probe_methods = ("load", "get", "create")
    else:
        candidates = (
            "kmeans", "dbscan", "gmm", "agglomerative",
            "fed_kmeans_numpy", "fed_kmeans_flower",
        )
        probe_methods = ("get", "create", "load")
    ...
    raise RuntimeError(
        ...
        "No public listing method or internal mapping was found."
    )
